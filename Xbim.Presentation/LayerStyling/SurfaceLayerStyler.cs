...
namespace Xbim.Presentation.LayerStyling
{
	public class SurfaceLayerStyler : ILayerStyler, IProgressiveLayerStyler
	{
		...
		private readonly XbimColourMap _colourMap = new XbimColourMap();
		...
		public SurfaceLayerStyler(LODs lod, ILogger logger = null)
		{
			Logger = logger ?? XbimLogging.CreateLogger<SurfaceLayerStyler>();
			_lod = lod;
			_colourMap = new XbimColourMap();
			InitColorMap(); 
		}

		protected void InitColorMap()
		{
			SolidColorBrush color =  GetLodColor(_lod);
			var xbimColors = _colourMap.Select(x => GetXbimColor(color, x.Name)).ToList();
			_colourMap.Clear();
			foreach (var xbimColor in xbimColors)
				_colourMap.Add(xbimColor);
		}

		public XbimScene<WpfMeshGeometry3D, WpfMaterial> BuildSceneLod(IModel model, XbimMatrix3D modelTransform,
			ModelVisual3D opaqueShapes, ModelVisual3D transparentShapes, List<IPersistEntity> isolateInstances = null, List<IPersistEntity> hideInstances = null, List<Type> excludeTypes = null)
		{
			...
			using (var geomStore = model.GeometryStore)
			{
				using (var geomReader = geomStore.BeginRead())
				{
					...
					{
						var wpfMaterial = GetWpfMaterialLod(model, styleId);
						...
						.Where(s => null == hiddenInstances || hiddenInstances.Count == 0 || !hiddenInstances.Keys.Contains(s.IfcProductLabel)))
					{
						...
						if (!materialsByStyleId.ContainsKey(styleId))
						{
							...
							var material2 = GetWpfMaterialByTypeLod(model, shapeInstance.IfcTypeId);
		...
		protected WpfMaterial GetWpfMaterialLod(IModel model, int styleId)
		{
		...
		protected WpfMaterial GetWpfMaterialByTypeLod(IModel model, short typeid)
		{
		...
		private XbimColour GetXbimColor(SolidColorBrush brush, string name)
		{
			if (brush != null)
			{
				Color color = brush.Color;
				return new XbimColour(name, color.R, color.G, color.B, color.A);
			}
			return null;
		}

		private SolidColorBrush GetLodColor(LODs lod)
		{
			SolidColorBrush brush = null;
			switch (lod)
			{
